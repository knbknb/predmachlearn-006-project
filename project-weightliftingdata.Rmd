...
> Predicting the quality of physical exercising by means of automated "Human Activity Recognition". This means analysis of sensor data acquired from 6 men doing biceps-curls with dumbbells. The sensor data can be used to tell *how correct* the movements of the workout were done.  
...
> For performance reasons, I used machine-learning toolbox *RWeka* (not *caret*) to run a tree-based algorithm *(J48)* on the 19600-row training data. The goal was to predict unknown values of the classification-attribute of a testing dataset with 20-rows. 

> **Result**: In a 10-fold leave-one-out cross-validation run with default parameters, the J48 model achieved a classification accuracy of 99%, and this sufficed to get 17 out of 20 test cases right (according to the Coursera grader).
...
Further exploratory analysis indicates that there are no outliers, at least not among in the most interesting parameters, to be described below in section "Exploratory Analyis 3".

To illustrate, a 5x5 feature-plot indicates that some users probably did not perform all types of "wrong" movements. 
...
For example, 3 users "rolled their belt", i.e. *leaned forward* to some degree, and 3 didn't.
```{r results="hide", echo=FALSE}

p <- qplot( roll_belt, user_name, data=training, geom=c("jitter" ), colour="yellow") + theme(legend.position="none")
p


```
This corresponds to the second-from-left plot in the last row of the plotmatrix.  
...
Here, training dataset and testing dataset differ in number of columns (1 column), and some colums differ in datatypes (logicals vs factors).
I'll temporarily append the tiny testing dataset (n=20) to the training data (n ~ 19600) to do the subsequent attribute filtering only once.
...
```
There are neqcol = `r length(comp2factinds) ` columns with different column types.  
Convert them:
```{r}
...
In the assignment's synopsis it says, the sensors were placed on *belt, forearm, arm*, and *dumbbell*. The sensors register
...
Let's filter out the relevant variables from the dataset, by building a complex regular expression for matching relevant column names:
...
contents(training)
```
These `r ncol(training)` attributes will be used for building a machine learning model.  
Note that there are only 20 NAs in the "classe" attribute. These are the appended test cases.

All attributes are numeric, and thus well suited for many algorithms.  

However, a feature plot (plotmatrix) for *all* `r ncol(training)` attributes and *all* `r nrow(training)` rows takes a few minutes to build.  
...
Passing in the training data with the appended testing data removed, using default options: 
```{r }
library(RWeka)
#testdata <- tail(training,20)
#try(write.arff(testdata, "testing-transformed-50attrs.arff", eol = "\n"))
#traindata <- training[-20,]
#try(write.arff(traindata, "training-transformed-50attrs.arff", eol = "\n"))
m1 <- J48(classe ~ ., data = traindata)

#J48(formula, data, subset, na.action,
#    control = Weka_control(), options = NULL)
```

#### Summary of the J48 model.
```{r}
summary(m1)
```


The model accuracy is `r round(summary(m1)$details["pctCorrect"],2)`% - not bad for a first run.  

The predictions for the 20 test cases are:
```{r}

pred1 <- predict(m1, newdata=tail(training,20)); tail(pred1, 20)
...
### 'Bagging' machine learning model.
Then I used a *Bagging*  algorithm (Breiman, 1996).  

Passing in the training data with the appended testing data removed, using default options: 
```{r }
library(RWeka)
m2 <- Bagging(classe ~ ., data = training[-20,])
...
The model accuracy is `r round(summary(m2)$details["pctCorrect"],2)`% - also not bad.  

The predictions for the 20 test cases are slightly different this time:
```{r}

pred2 <- predict(m2, newdata=testdata; tail(pred2, 20)


```
Only element `r which(pred1 != pred2)`, "B", is different. I submitted this to the Coursera grader, and got it right. 


I checked a bunch of other algorithms, e.g. "Logisitic, SMO, NaiveBayes*, all with default options, but none performed better.
...
The following is irrelevant to the readers. However
